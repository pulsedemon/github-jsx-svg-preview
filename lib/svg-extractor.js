  ...
  function getCodeLines(fileContainer) {
    ...
    if (codeEls.length > 0) {
      for (const code of codeEls) {
        ...
        if (code.closest('div[id^="diff-"]') !== fileContainer) continue;
    ...
    for (const cell of blobCells) {
      ...
      const closestFile = cell.closest('.file[data-tagsearch-path], .file[data-file-path]');
      if (closestFile && closestFile !== fileContainer) continue;
