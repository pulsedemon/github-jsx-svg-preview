    ...
    for (const line of lines) {
      ...
      if (!collecting) {
        ...
        if (svgStart !== -1) {
          ...
          depth = 0;
          buffer = [fragment];
          depth += countOpening(fragment, 'svg') - countClosing(fragment, 'svg') - countSelfClosing(fragment, 'svg');
      ...
      if (collecting) {
        buffer.push(trimmed);
        depth += countOpening(trimmed, 'svg') - countClosing(trimmed, 'svg') - countSelfClosing(trimmed, 'svg');
