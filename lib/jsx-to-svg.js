...
const JsxToSvg = (() => {
  ...
  function convert(jsxSvg) {
    ...
    // Remove JSX spread expressions: {...props}, {...rest}
    svg = svg.replace(/\{\.\.\.[\w.]+\}/g, '');

    // Remove JSX expression attributes: attr={expression}
    // but keep attr="string" and attr='string'
    svg = svg.replace(/(\w[\w-]*)=\{[^}]*\}/g, (match, attrName) => {
      // If it's a boolean-like JSX attr with expression, remove entirely
      return '';
    });

    // Remove standalone JSX expressions (children): {someVar}
    svg = svg.replace(/\{[^}]*\}/g, '');
  ...
  return { convert, toElement, CAMEL_TO_KEBAB_ATTRS };
})();
